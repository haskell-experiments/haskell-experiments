...
instance Arbitrary (a :~: (a -> a)) where
    arbitrary = undefined
    shrink = undefined
...
data Expr' t v where
    ...
    Lam :: Scope () (Expr' Int) v -> Expr' (Int -> Int) v
...
instance Generic (Expr' t v) where
    type Rep (Expr' t v) = 
            ...
            :+: (K1 R (t :~: (Int -> Int)) :*: K1 R (Scope () (Expr' Int) v))
            :+: (K1 R (t :~: Int) :*: K1 R (Expr' (Int -> Int) v) :*: K1 R (Expr' Int v))
    ...
    from (App e0 e1)   = R1 $ R1 $ R1 $ R1 $ R1 $ K1 Refl :*: K1 e0 :*: K1 e1
    ...
    to (R1 (R1 (R1 (R1 (L1 (K1 Refl :*: K1 e0)))))) = Lam e0
    to (R1 (R1 (R1 (R1 (R1 (K1 Refl :*: K1 e0 :*: K1 e1)))))) = App e0 e1
...
instance Generic1 (Expr' t) where
    type Rep1 (Expr' t) = 
            ...
            :+: (K1 R (t :~: (Int -> Int)) :*: S1 M (Scope () (Expr' Int)))
            :+: (K1 R (t :~: Int) :*: S1 M (Expr' (Int -> Int)) :*: S1 M (Expr' Int))
    ...
    from1 (App e0 e1)   = R1 $ R1 $ R1 $ R1 $ R1 $ K1 Refl :*: M1 e0 :*: M1 e1
    ...
    to1 (R1 (R1 (R1 (R1 (L1 (K1 Refl :*: M1 e0)))))) = Lam e0
    to1 (R1 (R1 (R1 (R1 (R1 (K1 Refl :*: M1 e0 :*: M1 e1)))))) = App e0 e1
...
instance Show v => Show (Expr' t v) where
    ...
    show (App e0 e1) = printf "App (%s) (%s)" (show e0) (show e1)
...
App e0 e1 `subst` f = App (e0 `subst` f) (e1 >>= f)
Lam (Scope e) `subst` f = Lam (Scope $ e >>= _Right f)
...
instantiate (Scope e) s = e >>= either (const s) pure
...
eval m (App e0 e1) = eval m . applyFun e0 =<< eval m e1
...
applyFun :: Expr' (Int -> Int) v -> Int -> Expr' Int v
applyFun (Lam e) = instantiate e . Lit

data Stack n where
    Null :: Stack 'Z
    (:-) :: Int -> Stack n -> Stack ('S n)

stackToList :: Stack n -> [Int]
stackToList Null = []
stackToList (x :- xs) = x : stackToList xs

instance Show (Stack n) where
    show = show . stackToList
...
exec :: Map String Int
     ...
     -> Stack n 
     -> Either [String] Int
exec m = exec' (exec m) Left Right m

traceExec :: Map String Int
          -> Code n 
          -> Stack n 
          -> IO (Either [String] Int)
traceExec m = exec' (trace m) (return . Left) (return . Right) m
    where
        trace m' c s = do
            print s
            print c
...
exec' :: (forall m. Code m -> Stack m -> r)
      -> ([String] -> r)
      -> (Int -> r)
      -> Map String Int
      -> Code n 
...
compile' m fetch (App (Lam (Scope e0)) e1)   = compile' m fetch e1 . compile' (sSucc m) (makeFetch (bumpFetch m fetch) m) e0 . RETURN
    where
        -- fetch' :: _
...
instance Arbitrary Operator where
    arbitrary = elements [minBound .. maxBound]
instance Arbitrary a => Arbitrary (Expr' (Int -> Int) a) where
    arbitrary = Lam <$> arbitrary
    shrink (Lam e) = Lam <$> shrink e
    ...
    arbitrary = sized $ \n -> do
            oneof $ 
                ...
                ++ if n == 0 then []
                         ...
                         , resize (n `div` 2) $ App <$> arbitrary <*> arbitrary
...
main = do 
        ...
        let a = Var "a"
            ...
            store' = fromList [("",1)]
            e = App (Lam (Scope (Add (Var (Left ())) (Var (Right ""))))) (Lit 0)
        print $ eval store $ "a" ! (3 * a + b) # 0
        print $ compile $ "a" ! (3 * a + b) # 0
        putStrLn "--------"
        ...
        print $ compile e
        putStrLn ""
        _ <- traceExec store' (compile e) Null
        return ()
